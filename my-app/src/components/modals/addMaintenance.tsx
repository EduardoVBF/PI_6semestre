...
export default function AddPreventiveMaintenanceModal({
  ...
  useEffect(() => {
    ...
    const fetchVehicles = async () => {
      try {
        ...
        // adaptar conforme formato da sua API (res.data.vehicles ou res.data)
  ...
  // travar placa se estiver na URL /vehicle/...
  useEffect(() => {
    if (!isOpen) return;
  ...
  // handle change (checkbox, select, input)
  const handleChange = (
    ...
    if (type === "checkbox") {
      ...
      setFormData((prev) => ({ ...prev, [name]: checked } as TMaintenanceForm));
      return;
    }

    const value = (e.target as HTMLInputElement).value;
    setFormData((prev) => ({ ...prev, [name]: value } as TMaintenanceForm));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-500/60 backdrop-blur-sm flex justify-center items-center p-4">
      <div
        className="w-full max-w-md bg-gray-800 p-8 rounded-xl shadow-xl relative max-h-[98dvh] overflow-y-auto"
